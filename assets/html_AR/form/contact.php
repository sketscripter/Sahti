...
### Success Messages
$msg_success = "We have <strong>successfully</strong> received your message.";
...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["contact-email"]) && $_POST["contact-email"] != '' && isset($_POST["contact-name"]) && $_POST["contact-name"] != '') {
		...
		$cf_symptom = isset($_POST["contact-symptom"]) ? $_POST["contact-symptom"] : '';
		$cf_age = isset($_POST["contact-age"]) ? $_POST["contact-age"] : '';
		...
		if ($honeypot == '' && !(empty($emailTO))) {
			...
			$bodymsg .= isset($cf_symptom) ? "Symptoms: $cf_symptom<br><br>" : '';
			$bodymsg .= isset($cf_age) ? "Age: $cf_age<br><br>" : '';
			$bodymsg .= isset($cf_message) ? "Message: $cf_message<br><br>" : '';
			$bodymsg .= $_SERVER['HTTP_REFERER'] ? '<br>---<br><br>This email was sent: ' . $_SERVER['HTTP_REFERER'] : '';
